	...
	case CourseActionMsg:
		...
		if msg.Error != nil {
			...
			if msg.Action == "transcript" {
				...
				// Go back to course detail view on error
			...
			if msg.Action == "transcript" {
				...
				// Successfully fetched attendance - go to attendance view
				m.currentView = AttendanceView
			} else if msg.Action == "assessments" {
				// Handle assessments view (you may need to create this)
				m.currentView = CourseDetailView // Or create AssessmentsView
			} else {
				m.currentView = CoursesView
			}
		}
